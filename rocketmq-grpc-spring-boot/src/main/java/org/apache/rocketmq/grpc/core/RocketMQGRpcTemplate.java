package org.apache.rocketmq.grpc.core;
...
public class RocketMQGRpcTemplate extends AbstractMessageSendingTemplate<String> implements DisposableBean {

    private static final Logger log = LoggerFactory.getLogger(RocketMQGRpcTemplate.class);
    ...
    private MessageConverter rocketMQMessageConverter = new MessageConverter();
    ...
    public MessageConverter getRocketMQMessageConverter() {
        return rocketMQMessageConverter;
    }

    public void setRocketMQMessageConverter(MessageConverter messageConverter) {
        this.rocketMQMessageConverter = messageConverter;
    }
    ...
    public CompletableFuture<SendReceipt> asyncSend(String destination, Message<?> message, Duration messageDelayTime) {
        ...
        Producer grpcProducer = this.getProducer();
        try {
            org.apache.rocketmq.client.apis.message.Message rocketMsg = this.createRocketMqgRPCMessage(destination, message, messageDelayTime, null);
    ...
    private org.apache.rocketmq.client.apis.message.Message createRocketMqgRPCMessage(String destination, Message<?> message, Duration messageDelayTime, String messageGroup) {
        Message<?> msg = this.doConvert(message.getPayload(), message.getHeaders(), null);
        return RocketMQUtil.convertToGRPCMessage(getMessageConverter(), charset,
